...
struct ExportSheet: View {
    ...
    var body: some View {
        VStack(spacing: 20) {
            ...
            VStack(alignment: .leading, spacing: 12) {
                exportOption(title: "JSON metadata only", icon: "doc.text")
                exportOption(title: "JSON with images", icon: "doc.zipper")
                exportOption(title: "Multi-file (folder)", icon: "folder")
            }
            
            Spacer()
            
            AppButton(title: "Close", variant: .secondary) {
                dismiss()
            }
        }
        .padding()
    ...
    private func exportOption(title: String, icon: String) -> some View {
        HStack {
